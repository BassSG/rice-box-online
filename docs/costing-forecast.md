# Rice Box Online Costing and Forecast

## Planning Assumptions

- Selling hours: 09:00-15:00.
- Kitchen start time: 09:00 after returning from school drop-off.
- Child pickup block: 15:00-16:30.
- Ingredient shopping time: after 16:30 for the next selling day.
- Operating days: 26 days per month.
- Starting volume: 30 boxes per day.
- Platform deduction assumption: 32.1% of platform selling price.
- Daily shopping budget for 30 boxes: about 1,600 THB plus 400 THB buffer.

The platform assumption is conservative. Always check the real contract before setting the final menu price.

## Menu Cost Sheet

| Menu | Estimated cost | Direct price | Platform price | Net after platform fee | Gross profit on platform |
| --- | ---: | ---: | ---: | ---: | ---: |
| ข้าวกะเพราหมูสับไข่ดาว | 41 THB | 69 THB | 99 THB | 67 THB | 26 THB |
| ข้าวหมูผัดน้ำมันหอย | 39 THB | 75 THB | 109 THB | 74 THB | 35 THB |
| ข้าวผัดหมู | 37 THB | 69 THB | 99 THB | 67 THB | 30 THB |
| ข้าวหมูกระเทียมไข่ดาว | 40 THB | 75 THB | 109 THB | 74 THB | 34 THB |

## Add-On Pricing

| Add-on | Estimated cost | Direct price | Platform price |
| --- | ---: | ---: | ---: |
| Fried egg | 5 THB | 12 THB | 18 THB |
| Extra rice | 4 THB | 10 THB | 15 THB |
| Extra pork | 13-17 THB | 25 THB | 35 THB |
| Special size | 10-15 THB | 20 THB | 29 THB |

## Evening Purchase Plan for Next-Day 30 Boxes

Buy this list after 16:30 at Makro, market, or the packaging supplier. The ingredients are for the next selling day, not the same morning.

| Item | Quantity | Budget |
| --- | ---: | ---: |
| Rice | 3.3 kg | 125 THB |
| Pork | 2.9 kg | 500 THB |
| Eggs | 26 eggs | 135 THB |
| Herbs and aromatics | 1 large set | 160 THB |
| Vegetables | 4-5 kg | 250 THB |
| Sauces, oil, seasoning | Daily average | 220 THB |
| Packaging | 35 sets | 230 THB |
| Utilities and small supplies | Daily average | 180 THB |
| Buffer | Reserve | 200 THB |
| Total cash to carry |  | 2,000 THB |

## Scale Plan

| Volume | Suggested cash to carry | Staff need | Notes |
| --- | ---: | --- | --- |
| 30 boxes/day | 2,000 THB/day | Owner only, possibly 1 part-time helper | Good pilot size. |
| 60 boxes/day | 3,600-4,000 THB/day | Owner plus 1 helper | Add separate packing station. |
| 100 boxes/day | 6,000-6,500 THB/day | Owner plus 2 helpers or prep support | Needs stronger route planning and inventory control. |

## Startup Budget Packages

These packages assume the home kitchen already has main equipment such as stove, pans, rice cooker, knives, and basic storage.

| Package | Prepare | Target Volume | What to Buy First |
| --- | ---: | ---: | --- |
| Low | 12,000-18,000 THB | 15-20 boxes/day | Ingredients for 3 days, 100-150 packaging sets, 5 days of working cash, 2,500-3,000 THB emergency reserve. |
| Mid | 28,000-40,000 THB | 30-40 boxes/day | Ingredients for 5-7 days, 250-300 packaging sets, 10-14 days of working cash, small marketing budget, 5,000 THB emergency reserve. |
| High | 55,000-75,000 THB | 50-70 boxes/day | Ingredients for 10-14 days, about 500 packaging sets, 3-4 weeks of working cash, larger marketing budget, 10,000 THB emergency reserve. |

Do not buy bulk packaging or extra equipment before the menu sells repeatedly. Cash flow is more important than looking big.

## Monthly Forecast

| Period | Boxes/day | Boxes/month | Gross sales | Net after platform fees | Variable cost | Fixed cost | Cash before tax |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Month 1 | 30 | 780 | 71,100 THB | 55,500 THB | 31,200 THB | 12,000 THB | 12,300 THB |
| Month 3 | 60 | 1,560 | 143,500 THB | 112,300 THB | 58,500 THB | 24,000 THB | 29,800 THB |
| Month 6 | 100 | 2,600 | 247,000 THB | 195,000 THB | 94,000 THB | 42,000 THB | 59,000 THB |

## Weekly Review Questions

1. Which menu has the highest profit per minute of cooking time?
2. Which menu creates the most waste?
3. How many orders came from direct customers?
4. How much platform fee was paid?
5. Did the shop run out of any ingredient?
6. Did any customer complain about delivery time or food quality?

## Pricing Rule

Use this rule when adding new menu items:

1. Calculate ingredient cost.
2. Add packaging cost.
3. Add gas/electric/water estimate.
4. Add waste buffer.
5. Set direct price with at least 35-45% gross margin.
6. Set platform price higher enough to survive platform deduction.

Simple formula:

`Platform price = Direct price / 0.679`

Then round up to a customer-friendly price such as 99, 109, 119, or 129 THB.

## References to Recheck Weekly

- Grab Thailand explains that merchant commission can vary and average around 15-30% of order value: https://www.grab.com/th/en/blog/askgrab-merchant-commission/
- GrabFood merchant terms note that service fee and VAT details are governed by the merchant contract: https://www.grab.com/th/en/terms-policies/general-terms-and-conditions-of-grabfood-merchant-contract/
- foodpanda partner information notes entry fee, monthly fee, commission on sales, and VAT exclusion from commission: https://www.thainamthip.co.th/en/food-service/foodpanda
- CheckRaka aggregates Thai restaurant ingredient prices from public sources and updates frequently: https://checkraka.app/
